...
int bloom_lookup(struct bloom * this, char * item) {
  int hash_1=hash_string(item, BLOOM_HASH1, this->size);
  int hash_2=hash_string(item, BLOOM_HASH2, this->size);
  if(bitset_lookup(this->bitset, hash_1) && bitset_lookup(this->bitset, hash_2)) {
    return 1;
  }
  else{
    return 0;
...
void bloom_add(struct bloom * this, char * item) {
  int hash_1=hash_string(item, BLOOM_HASH1, this->size);
  int hash_2=hash_string(item, BLOOM_HASH2, this->size);
  bitset_add(this->bitset, hash_1);
  bitset_add(this->bitset, hash_2);
}
